...
import { ExternalLink, Pencil, Check, Flame, Star, RotateCcw } from "lucide-react";
...
const GRADES = [
  {
    ...
    icon: RotateCcw,
    classes:
      "border-rose-500/25 text-rose-400 hover:border-rose-500/60 hover:bg-rose-500/10",
  },
  {
    key: "hard",
    label: "Hard",
    ...
    icon: Flame,
    classes:
      "border-amber-500/25 text-amber-400 hover:border-amber-500/60 hover:bg-amber-500/10",
  },
  {
    key: "good",
    label: "Good",
    ...
    icon: Check,
    classes:
      "border-emerald-500/25 text-emerald-400 hover:border-emerald-500/60 hover:bg-emerald-500/10",
  },
  {
    key: "master",
    label: "Master",
    ...
    icon: Star,
    classes:
      "border-violet-500/25 text-violet-300 hover:border-violet-500/60 hover:bg-violet-500/10",
  },
];
...
export default function ReviewCard({ problem = {}, onRate, onNotesChange }) {
  ...
  const [syntaxDraft, setSyntaxDraft] = useState(problem.syntax || "");
  ...
  useEffect(() => {
    ...
    setSyntaxDraft(problem.syntax || "");
  }, [problem.notes, problem.syntax]);

  useEffect(() => {
    const detectLanguage = (code) => {
      if (!code) return "text";
      const text = code.toLowerCase();
      if (/^\s*#include|std::|cout|cin/.test(text)) return "cpp";
      if (/function\s+|\bconst\b|\blet\b|=>|console\.log/.test(text)) return "javascript";
      if (/^\s*def\s+|import\s+|print\(|self\.|:\s*$/.test(text)) return "python";
      if (/public\s+static\s+void\s+main|system\.out\.println/.test(text)) return "java";
      if (/select\s+.+\s+from|insert\s+into|update\s+.+set/.test(text)) return "sql";
      if (/^\s*\$[a-z0-9_]+\s*=|echo\s+\$/.test(text)) return "php";
      return "text";
    };

    try {
      const lang = detectLanguage(syntaxDraft);
  ...
  const handleSaveNotes = async () => {
    ...
    const payload = { notes: notesDraft, syntax: syntaxDraft };
    const sameNotes = notesDraft === (problem.notes || "");
    const sameSyntax = syntaxDraft === (problem.syntax || "");
    if (sameNotes && sameSyntax) {
      setEditing(false);
      return;
    }

    setSaving(true);
    const result = await onNotesChange(problem.id, payload);
    if (result?.error && payload.syntax !== undefined) {
      await onNotesChange(problem.id, notesDraft);
    }
    setSaving(false);
    setEditing(false);
  };
  ...
  const syntaxPreview = syntaxDraft ? (
    <pre className="mt-3 overflow-x-auto rounded-2xl bg-slate-950 p-4 text-sm text-slate-100"><code>{syntaxDraft}</code></pre>
  ) : (
    ...
    <div className="card rounded-[24px] border border-white/10 bg-slate-950/80 p-6 shadow-[0_30px_70px_-35px_rgba(15,23,42,0.9)]">
      ...
      <div className="mt-6 rounded-3xl border border-white/5 bg-slate-900/80 p-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-white">Study view</p>
            <p className="text-xs text-slate-500">Reveal notes only after recall.</p>
          </div>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setRevealedNotes((v) => !v)}
              className="rounded-full border border-violet-500 px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-violet-300 transition hover:bg-violet-500/10"
            >
              {revealedNotes ? "Hide notes" : "Reveal notes"}
            </button>
            <button
              type="button"
              ...
              className="rounded-full border border-violet-500 px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-violet-300 transition hover:bg-violet-500/10"
            >
              {revealedSyntax ? "Hide syntax" : "Reveal syntax"}
            </button>
          </div>
        </div>

        <div className="mt-6 space-y-5">
          {!editing && !revealedNotes && !revealedSyntax && (
            <div className="mt-2 rounded-3xl border border-dashed border-white/10 bg-slate-950/70 p-8 text-center text-sm text-slate-400">
              Notes and syntax are hidden until you click Reveal. Try to recall the problem and then review your saved approach.
            </div>
          )}
            <div>
              <div className="flex items-center justify-between gap-4">
              ...
              {editing ? (
                ...
                <div className="mt-4 rounded-3xl border border-dashed border-white/10 bg-slate-950/70 p-6 text-center text-sm text-slate-400">
                  Notes are hidden. Click "Reveal notes" to show your saved approach.
                </div>
              ) : (
            ...
            <div>
              ...
              {editing ? (
                ...
                <div className="mt-4 rounded-3xl border border-dashed border-white/10 bg-slate-950/70 p-6 text-center text-sm text-slate-400">
                  Syntax is hidden. Click "Reveal syntax" to show any saved snippets.
                </div>
              ) : (
                <div className="mt-4 rounded-3xl border border-white/10 bg-slate-950/80 p-4">
                  <div className="flex items-center justify-between">
                    <div className="text-xs uppercase tracking-[0.12em] text-slate-400">Detected</div>
                    <div className="text-xs font-mono text-slate-200">{detectedLang}</div>
                  </div>
                  {syntaxPreview}
                </div>
              )}
